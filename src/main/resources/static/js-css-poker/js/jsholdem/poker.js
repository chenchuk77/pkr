...
function handle_end_of_round () {
  ...
  if (human_loses == 1) {
    ...
    if (num_playing < 2) {
      ...
      ending = NUM_ROUNDS == 1 ? "1 deal." : NUM_ROUNDS + " deals.";
      if (has_money(0)) {
...
function the_bet_function (player_index, bet_amount) {
  if (players[player_index].status == "FOLD") {
    ...
    old_current_bet = current_bet_amount;
    current_bet_amount = players[player_index].subtotal_bet + bet_amount;

    if (get_pot_size() > 0) {
      current_min_raise = current_bet_amount - old_current_bet;
...
function get_valid_bet(amount, minimum, maximum, prompt_text) {
  ...
  while (iterate) {
    ...
    var as_integer = parseInt(amount, 10);
    gui_log_to_history("Amount " + amount);
    gui_log_to_history("As_int " + as_integer + " max is " + maximum);
  ...
  gui_log_to_history("Accepted " + as_integer);
  return as_integer;
}
